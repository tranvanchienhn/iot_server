...
public class CustomerController extends BaseController {
    ...
    //12.4.2025: Trí sửa thêm customer, lấy theo tenantId duy nhất & không cần xác thực người dùng
...
@RequestMapping(value = "/noauth/customer", method = RequestMethod.POST)
@ResponseBody
public Customer registerCustomer(@io.swagger.v3.oas.annotations.parameters.RequestBody(description = "A JSON value representing the customer.") @RequestBody Customer customer) throws Exception {


    PageData<Tenant> tenants = tenantService.findTenants(new PageLink(1));
    if (tenants == null || tenants.getData().isEmpty()) {
        throw new IllegalStateException("Khong tim thay tenant nao trong he thong!");
    }

    Tenant firstTenant = tenants.getData().get(0);
    customer.setTenantId(firstTenant.getId());
    log.debug("Tao moi customer cho tenant: {}", firstTenant.getId());
    return tbCustomerService.save(customer);
}
...
@PreAuthorize("hasAuthority('TENANT_ADMIN')")
@GetMapping("/tenant/customers/all")
public List<Customer> getAllCustomers(@RequestParam(required = false) String textSearch) {
   
    try {
        TenantId tenantId = getCurrentUser().getTenantId();
    
        return customerService.findAllCustomersByTenantId(tenantId);
        // Xử lý user ở đây
