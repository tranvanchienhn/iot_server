...
public class UserController extends BaseController {
    ...
    public User saveUser(
            ...
            @RequestParam(required = false, defaultValue = "true") boolean sendActivationMail, HttpServletRequest request) throws ThingsboardException {
                ...
                // PageData<Tenant> tenants = tenantService.findTenants(new PageLink(1));
                // if (tenants == null || tenants.getData().isEmpty()) {
                //     throw new IllegalStateException("Không tìm thấy Tenant nào trong hệ thống!");
                // }
                // Tenant firstTenant = tenants.getData().get(0);
                // user.setTenantId(firstTenant.getId());
                // return tbUserService.registerNewUser(user.getTenantId(), user.getCustomerId(), user, sendActivationMail,request);
    }

    @RequestMapping(value = "/noauth/user", method = RequestMethod.POST)
    @ResponseBody
    ...
    //21.4.2025: Trí cho phép khách hàng nhận active email
    @PreAuthorize("hasAnyAuthority('SYS_ADMIN', 'TENANT_ADMIN','CUSTOMER_USER')")
    ...
    //25.04.2025 Trí thêm activationLink cho khách hàng đăng ký mới
    @GetMapping(value = "/noauth/user/{tenantId}/{userId}/{customerId}/activationLink1", produces = "text/plain")
    @ResponseBody
    public String getActivationLink1(@Parameter(description = USER_ID_PARAM_DESCRIPTION)
    @PathVariable(TENANT_ID) String strTenantId,
                                    @PathVariable(USER_ID) String strUserId,
                                    @PathVariable(CUSTOMER_ID) String strCustomerId,
                                    HttpServletRequest request) throws ThingsboardException {
        return getActivationLinkInfo1(strTenantId, strUserId, strCustomerId,request).value();
    }

    @GetMapping(value = "/noauth/user/{tenantId}/{userId}/{customerId}/activationLinkInfo1")
    public UserActivationLink getActivationLinkInfo1(@Parameter(description = USER_ID_PARAM_DESCRIPTION)
    @PathVariable(TENANT_ID) String strTenantId,
                                                    @PathVariable(USER_ID) String strUserId,
                                                    @PathVariable(CUSTOMER_ID) String strCustomerId,
                                                    HttpServletRequest request) throws ThingsboardException {
        ...
        return tbUserService.getActivationLink(tenantId, customerId, userId, request);
    }
