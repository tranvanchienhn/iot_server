...
public class CustomerServiceImpl extends AbstractCachedEntityService<CustomerCacheKey, Customer, CustomerCacheEvictEvent> implements CustomerService {
    ...
    //Trí - 30-03-2025
    @Override
public List<Customer> findAllCustomersByTenantId(TenantId tenantId) {
    List<Customer> customers = new ArrayList<>();
    PageLink pageLink = new PageLink(1000); // Chia nhỏ nếu cần
    PageData<Customer> pageData;
    
    do {
        pageData = findCustomersByTenantId(tenantId, pageLink);
        customers.addAll(pageData.getData());
